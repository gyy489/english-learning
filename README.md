...
5. 只有目标词不超过 7 个、本篇生词不超过 5 个、没有到期顺延且收件箱不超过 15 个时，才读取 `雅思真题/Markdown资料/7月阅读/ReadingPractice/readingpractice-index.json`；否则完全不读取题库，只围绕目标词生成基础词汇文章。
...
这个目录是低负荷时的真题语料来源。只有单词复习系统允许使用雅思原文时，才应优先读取这些 Markdown，而不是临时重新解析 PDF；高负荷时完全不读取题库。
