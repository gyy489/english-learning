...
4. 第二天写新文章时，系统先安排这些生词，再从长期词库中补充少量到期旧词，让 AI 在新文章中自然使用它们。
5. 每篇目标复习词不超过 15 个；复习压力较低时才从雅思真题语料中加入少量新词，压力较高时只做旧词复习。
...
3. 先安排前一天仍不会的词，再补充到期历史词；总目标词上限为 15：
   - 当前生词 0-5 个：最多补 5 个到期旧词，并允许 3-5 个新 IELTS 目标词。
   - 当前生词 6-10 个：最多补 4 个到期旧词，并只允许 1-2 个新 IELTS 目标词。
   - 当前生词 11 个以上：不补到期旧词，不主动引入新目标词；超过 15 个的当前生词顺延。
4. 到期旧词按最近再次遇到、逾期程度和遗忘次数排序；当天放不下的词继续保留在队列中，不会删除。只要队列里仍有到期积压，就暂停引入新目标词。
...
这个循环的逻辑是：我阅读文章时，把还没掌握的单词写到文章底部；下一次生成时，复习词如果没有再次被记录，就视为本次记住并延长间隔；如果再次记录，就视为仍不会并回到次日复习。系统同时从长期词库抽取到期旧词，直到它们经过多次间隔回忆后进入“基本熟悉”或“已掌握”。
...
- 每篇文章最多安排 15 个目标复习词；先处理当前仍不会的词，再补到期旧词，超出容量的词自动顺延。
